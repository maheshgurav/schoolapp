...
public class ExaminationController {
	...
	public String loadSubjectAndMarks(HttpServletRequest request,
		...
		try {
			...
			Map<String, String> subjectGuidAndName = new HashMap<String, String>();
			List<String> subjectDataPresent = new ArrayList<String>();
			while (st.hasNext()) {
				Map<?, ?> map = (Map<?, ?>) st.next();
				subjects.add(map.get("subject").toString());
				subjectGuidAndName.put(map.get("subject").toString(),
			...
			List<Subject> subjectsList = ModelAttributesCommon.getSubjectsForModelAttributes();
			Map<String,String> subjectCodeMap = new HashMap<String, String>();
			for (Subject subject : subjectsList) {
				subjectCodeMap.put(subject.getGuid(),subject.getCode());
			}

			for (Object object : examResult) {
				ExamResult examResultObj = (ExamResult) object;
				JSONObject obj = new JSONObject();
				obj.put("subject_name", subjectGuidAndName.get(examResultObj
						.getExamResultKey().getSubjectGuid()));
				obj.put("subject_code", subjectCodeMap.get(examResultObj.getExamResultKey()
						.getSubjectGuid()));
			...
			for (String subjectPresent : subjects) {
				if (!subjectDataPresent.contains(subjectPresent)) {
					...
					obj.put("subject_name",
							subjectGuidAndName.get(subjectPresent));
					obj.put("subject_code", subjectCodeMap.get(subjectPresent));
					obj.put("marks", "0");
					array.put(obj);
				}
			}
	...
	private Map<String, String> getSubjectNameWithCode(List<Subject> subjects) {
		...
		for (Subject subject : subjects) {
			result.put(subject.getGuid(), subject.getName());
		}
		return result;
	}
	...
	public String loadStudents(HttpServletRequest request,
		...
		List<Object> studentSubjects = examinationControllerUtility.getStudents(
				classGuid, RequestCommon.getApplicationSession(request)
						.getSchoolGuid(), subjectGuid);

		List<String> studentGuids = new ArrayList<String>();
		for (Object object : studentSubjects) {
			StudentSubjectDetails studentSubjectDetails = (StudentSubjectDetails) object;
			studentGuids.add(studentSubjectDetails.getStudentId());
		}
		Map<String, String> examResult = examinationControllerUtility
				.getExamResult(studentGuids, subjectGuid, examGuid);

		List<Object> students = studentControllerUtility.getStudentsByGuid(studentGuids);
		JSONArray array = new JSONArray();
		try {
			for (Object object : students) {
				...
				obj.put("name", student.getName().toString());
	...
	public String getStudentReportCard(HttpServletRequest request,
		...
		try{
			...
			List<Object> reportCard = examinationControllerUtility.getReportCard(examGuid, studentGuid);
	        jasperPrint = reportsControllerUtility.fillJasperReport(reportCard, request, "STUDENT_REPORT_CARD" , parameterMap);
	        if (jasperPrint != null) {
